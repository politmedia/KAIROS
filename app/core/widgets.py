    ...
    def render(self, name, value, attrs=None,):
        ...
        substitutions = {
            ...
            'initial': '<img class="img-responsive img-thumbnail" width="%s" src="%s">' % (
                force_text('100%'),
                force_text(get_thumbnailer(value)['medium'].url if value and hasattr(
                    value, 'url') else static('images/placeholder.svg'))
            )
        }
        ...
        if not self.is_required:
            ...
            substitutions['clear_template'] = self.clear_checkbox_name(
                checkbox_name)
